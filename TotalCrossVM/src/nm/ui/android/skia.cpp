...
int skia_makeBitmap(int32 id, void *data, int32 w, int32 h) {
    ...
    if (id < 0) { // must create a new bitmap
        ...
        void* dstPixels = operator new(bitmap.info().width() * bitmap.info().height() * canvas->imageInfo().bytesPerPixel());
        ...
        bitmap.readPixels(dstImageInfo, dstPixels, bitmap.info().minRowBytes(), 0, 0);
